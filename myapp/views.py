...
def create(request):
    user = User.objects.get(id=request.session['user_id'])
    errors = List.objects.listValidator(request.POST)
    print(errors)
    ...
    if errors:
        ...
        lists = List.objects.create(item_product = request.POST['item_product'], added_by_id = request.session['user_id'])
        user.liked.add(lists)

        return redirect(f"/viewList/{lists.id}")
        
def viewList(request, lists_id):
    lists = List.objects.get(id=lists_id)
...
def delete(request, lists_id):
    lists_to_delete = List.objects.get(id=lists_id)	
